...
const LightsEffect = ({isMobile, computerIsNarrowScreenXOR}) => {
  ...
  return (
    <div ref={containerRef} style={{ height: 'max(200svw, 200svh)', position: 'absolute', width: '100%'}}>
      <div className='lightsWrapper' style = {{top:computerIsNarrowScreenXOR?'50vw':'35vw'}}>
        <motion.img 
        ...
        />
        <motion.img
          className = 'spotLightLogo' src = {clubXLogo} style = {{width:isMobile?'55vw':'20vw', bottom:isMobile ? !computerIsNarrowScreenXOR ?'28%':'15%':'52.5%', minWidth:isMobile?'200px':'400px'}}
        />
      </div>
    </div>
  );

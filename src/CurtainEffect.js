...
const CurtainEffect = ({blurCurtains = true, top = '0'} /*todo tee tällä paramilla jutaki jos halutaan verhoja muuallekki sivulle*/) => {
  ...
  //todo make this one const emt meni aivot kyl vähä solmuu
  ...
  return (
    <div ref={containerRef} style={{ height: 'max(150svw, 150svh)', position: 'absolute', width: '100%', left:0}}>
      <div className='curtainsWrapper'>
        <motion.img 
            className = 'curtain' src={CurtainLeft} style={{x: curtainProgressX_1, top:'-50px', filter: blurCurtains? curtainBlurRadius: 'none'}} alt = 'coolass curtain left side'
        />
        <motion.img
          className = 'curtain' src={CurtainRight} style = {{x: curtainProgressX_2, top:'-30px', filter: blurCurtains? curtainBlurRadius: 'none'}} alt = 'coolass curtain right side'
        />

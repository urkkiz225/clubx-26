  ...
  return (
    <div ref={containerRef} style={{ height: 'max(150vw, 150vh)', position: 'absolute', width: '100%', left:0}}>
      <div className='curtainsWrapper'>
        <motion.img 
          className = 'curtain' src={CurtainLeft} style={{x: curtainProgressXLeft, top:'-50px', filter: !isMobile? curtainBlurRadius: 'blur(8px)'}} alt = 'cool curtain left side'
        />
        <motion.img
          className = 'curtain' src={CurtainRight} style = {{x: curtainProgressXRight, top:'-30px', filter: !isMobile? curtainBlurRadius: 'blur(8px)'}} alt = 'cool curtain right side'
        />
